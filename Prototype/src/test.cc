...
/*查询sql为select * from table where + 条件语句 。条件语句只需考虑id的大小关系及or和and运算符
比如：select * from table where id > 1556 and id <5000
删除数据sql类似：delete from table where + 条件语句。条件语句要求与查询sql一致。
delete from table where id > 300 or id > 784
更新数据sql类似：update table set 《字段名》 = ？ where + 条件语句。 条件语句要求与查询sql一致。
比如：update table set number = 20157894 where id<900 or id>9950*/
struct RowData
{
	int id;
	string name;
