...
std::pair<int,int> BplusTree::search(struct iovec *record, size_t iovcnt) {//header=1代表叶节点 header=0代表非叶节点，遍历时通过栈记录路径
    ...
    std::cout<<"Leaf Block id: "<<blockid<<std::endl;
    ...
    for(int i = 0; i <block.getSlotsNum();i++) {//找到刚好比key大的，然后往下迭代找，直到找到叶子节
    ...
    int nextid=0;//返回两个，如果右边有就返回右边的，没有就回溯mystack，往回找，因为是左指针，所以是后继，
...
int BplusTree::insert(struct iovec *record, size_t iovcnt) {//header=1代表叶节点 header=0代表非叶节点；右指针nextid,正数代表非叶节点，负数代表叶节点
    ...
    if(!ret) {//分裂，插入父节点 //判断是否是root
        ...
        while(!ret) {
            ...
            int right=block.getSlotsNum()-left;
            ...
            if(flag==0)newblock.allocate(&header,record,(int)iovcnt);//是否比当前最小的小
            memcpy(buffer_,newbuffer,Block::BLOCK_SIZE);
            sortSlots(newblock,2);
            newoffset = (root.getGarbage()-1) * Block::BLOCK_SIZE + Root::ROOT_SIZE;
            indexfile_.write(newoffset, (const char *) buffer_, Root::ROOT_SIZE);
            ...
            if(count==0)nextid=-block.getNextid();//叶节点
            newblock.setNextid(nextid);//右指针
            ...
            if(flag==1)newblock.allocate(&header,record,(int)iovcnt);
            memcpy(buffer_,newbuffer,Block::BLOCK_SIZE);
            sortSlots(newblock,2);
            ...
            if(block.blockid()==root.getHead()){//父节点,如果当前是根节点，新开一个block作为新的根节点
                ...
                newoffset = (root.getGarbage()-1) * Block::BLOCK_SIZE + Root::ROOT_SIZE;
                indexfile_.write(newoffset, (const char *) newbuffer, Root::ROOT_SIZE);
                root.setGarbage(root.getGarbage()+1);
                indexfile_.write(0, (const char *) rb, Root::ROOT_SIZE);
                break;
            }
        ...
        sortSlots(block,2);//更新最后一个指针
        reoffset = block.getSlot(block.getSlotsNum()-1);
        getRecord(iov_,reoffset,iovcnt,recordbuffer,&header);
        int nextid=-*(int*)iov_[1].iov_base;
